...
import { StyleSheet, Text, View, RefreshControl, FlatList, ScrollView, TouchableOpacity, SafeAreaView, Alert } from 'react-native';
...
class SubService extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({ item, type: item.id, services: this.props.service.rows }, () => {
      this.init();
    });
  }
  ...
  render() {
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={{ border: 1, borderColor: '#000', marginTop: 10, marginLeft: 16 }}>
          <View style={{ flexDirection: 'row'}}>
            <Icon name={"search"} size={22} color={'#595959'} style={{marginTop: 10, marginLeft: 10}}/>
            <TextInput
              style={{ marginLeft: 4, marginBottom: 5, color: '#595959', fontSize: 18 }}
              ref="searchBar"
          ...
          <View style={{ borderWidth: 1 / 2, marginHorizontal: '1%', borderColor: '#595959', marginVertical: '1%', marginTop: '-2%', opacity: 0.1, marginLeft: '1%', marginRight: '4%'}} />
        </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity onPress={() => this.onPress(item)}>
              <View style={{ marginTop: 22, marginEnd: 10, margin: 4 }}>
                <Text style={{ textTransform: 'uppercase', fontWeight: '500', color: '#595959', fontSize: 14, fontWeight: 'bold', paddingLeft: 19, paddingTop: 22 }}>{item.name}</Text>
                <Text style={{ fontWeight: '500', color: '#595959', fontSize: 14, paddingLeft: 19 }}>{item.annotation}</Text>
                <View style={{ flexDirection: 'row', alignItems: 'center', paddingLeft: 19, paddingBottom: 22 }}>
                  <View style={{ marginLeft: '73%', marginTop: '-12%' }}>
                    {item.type.name.substr(0, 1) == "C" && (
                      <ProgressCircle
                        ...
                        bgColor="#828fff"
                      >
                        <Text style={{ fontSize: 14, fontWeight: 'bold', color: '#fff' }}>{item.name.substr(0, 1)}</Text>
                      </ProgressCircle>
                    )}
          ...
          ListEmptyComponent={() => this.props.service.isLoading ? null : <EmptyList message={"Não há dados disponíveis"} />}
        />
      </SafeAreaView>
    );
  }
...
export default connect(mapStateToProps, { serviceList, clear, current })(SubService)